...
class VariableController extends Controller
{
    ...
    public function add(Request $request)
    {
        ...
        $rules = Rule::distinct()->select('id')->where('controlaction_id', $control_action->id)->get();
    ...
    public function delete(Request $request)
    {
        ...
        foreach ($states as $state) {
            DB::select(DB::raw(
                "UPDATE context_tables SET context = REPLACE(context, ',".$state->id.",', ',')
                WHERE context like '%,".$state->id.",%'"
            ));
            DB::select(DB::raw(
                "UPDATE context_tables SET context = REPLACE(context, ',".$state->id."', '')
                WHERE context like '%,".$state->id."'"
