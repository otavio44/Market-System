...
class SafetyConstraintsController extends Controller
{
    ...
    public function refreshUcasWithRules(Request $request)
    {
        ...
        foreach ($safety_constraints as $safety_constraint) {
            if (!in_array($safety_constraint->type, $columns)) {
                RulesSafetyConstraintsHazards::where('rule_id', $rule->id)
                    ->where('sc_id', $safety_constraint->id)->delete();
    ...
    {
        $safety_constraints = SafetyConstraints::where('controlaction_id', $request->input('controlaction_id'))
            ->where('rule_id', 0)->get();
